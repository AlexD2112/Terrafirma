...
public class OldHexMap {
    ...
    public void renderMap(Batch batch, double width, double height, float recedeFactor, Vector2 mapScalarPoint, double zoom, double maxZoom) {
        double greyScale = 0;
        if (zoom > maxZoom) {
        ...
        for (Hexagon<SatelliteData> hexagon : grid.getHexagons()) {
            ...
            for (int i = 0; i < numPoints; i++) {
                ...
                Vector2 transformedPoint = DisplayFunctions.transformPoint(new Vector2((float) p.getCoordinateX(), (float) p.getCoordinateY()), width, height, recedeFactor, mapScalarPoint, zoom);
        ...
        //Make a square at mapScalarPoint
        ShapeDrawer squareDrawer = new ShapeDrawer(batch, whitePixelRegion);
        squareDrawer.setColor(Color.ORANGE);
        squareDrawer.filledRectangle((float) mapScalarPoint.x - 5, (float) mapScalarPoint.y - 5, 10, 10);

        //Make another square thats reverse transformation of mapScalarPoint
        ShapeDrawer square2Drawer = new ShapeDrawer(batch, whitePixelRegion);
        Vector2 reverseTransformedPoint = DisplayFunctions.reverseTransformation(mapScalarPoint, width, height, recedeFactor, mapScalarPoint, zoom);
        square2Drawer.setColor(Color.RED);
        square2Drawer.filledRectangle((float) reverseTransformedPoint.x - 5, (float) reverseTransformedPoint.y - 5, 10, 10);

        //Make another square thats transformation of mapScalarPoint
        ShapeDrawer square3Drawer = new ShapeDrawer(batch, whitePixelRegion);
        Vector2 transformedPoint = DisplayFunctions.transformPoint(mapScalarPoint, width, height, recedeFactor, mapScalarPoint, zoom);
        square3Drawer.setColor(Color.BLUE);
        square3Drawer.filledRectangle((float) transformedPoint.x - 5, (float) transformedPoint.y - 5, 10, 10);
    }
